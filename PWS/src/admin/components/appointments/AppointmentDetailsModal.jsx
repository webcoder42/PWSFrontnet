...
const AppointmentDetailsModal = ({ appointment: initialAppointment, onClose, onReschedule, onStatusUpdated }) => {
  ...
  const canComplete = normalizedStatus !== 'completed' && normalizedStatus !== 'cancelled';
  ...
  const handleMarkAsComplete = async () => {
    ...
    setCompleting(true);
    try {
      const response = await updateAppointmentStatusAPI(apptId, 'completed');
      if (response.success) {
        const updatedAppt = { ...appointment, status: 'Completed', rawStatus: 'completed' };
        setAppointment(updatedAppt);
        onStatusUpdated?.(updatedAppt);
      }
    } catch (err) {
          ...
          <div className="flex items-center gap-3">
            {canComplete && (
              <button
                ...
                disabled={completing}
                className="bg-emerald-500 hover:bg-emerald-600 px-4 py-2 rounded-lg text-white font-bold text-sm"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-8 p-8">
          ...
          <div className="rounded-[2rem] border border-gray-100 p-6 shadow-sm bg-white flex flex-col justify-between">
            ...
            <div className="space-y-4 mt-6">
              {/* Action buttons moved to header — keep this area for additional footer controls if needed */}
            </div>
          </div>
        </div>
      </div>
