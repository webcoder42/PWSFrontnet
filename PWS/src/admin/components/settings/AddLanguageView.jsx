...
const AddLanguageView = ({ setView }) => {
  const [selectedAddLang, setSelectedAddLang] = useState('Spanish');
  ...
  <div className="animate-fade-in max-w-4xl mx-auto pb-20">
    ...
    <div className="bg-white rounded-[2rem] border border-gray-50 shadow-sm overflow-hidden mb-12 divide-y divide-gray-25 h-[600px] overflow-y-auto custom-scrollbar">{['Spanish', 'Mandarin', 'Cantonese', 'Punjabi', 'Tagalog', 'Arabic', 'Italian', 'Portuguese', 'German', 'Korean', 'Vietnamese', 'Polish', 'Urdu', 'Hindi', 'Tamil', 'Persian', 'Russian', 'Japanese'].map(lang => (<button onClick={() => setSelectedAddLang(lang)} key={lang} className="w-full text-left p-6 flex items-center justify-between hover:bg-gray-25 transition-colors group"><span className={`text-sm font-bold ${selectedAddLang === lang ? 'text-purple-600' : 'text-gray-400 group-hover:text-gray-600'}`}>{lang}</span></button>))}</div>
    <div className="flex gap-4"><button onClick={() => setView('languages')} className="flex-1 py-5 bg-white border border-gray-100 rounded-[2rem] text-gray-500 font-bold text-sm hover:bg-gray-50 transition-all">Cancel</button><button onClick={() => setView('languages')} className="flex-1 py-5 bg-gradient-to-r from-purple-700 to-purple-500 text-white rounded-[2rem] font-bold text-sm shadow-xl shadow-purple-200 hover:-translate-y-1 transition-all">Save</button></div>
  </div>
)};
