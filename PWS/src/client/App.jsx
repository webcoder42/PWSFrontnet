...
function App() {
  ...
  const onNavigate = (tab) => {
    navigate(getNavigatePath(tab, PATIENT_BASE));
  };

  const activeTab = getActiveTabFromPath(location.pathname, PATIENT_BASE);
  ...
  return (
    <Layout activeTab={activeTab} onTabChange={onNavigate}>
      <Routes>
        ...
        <Route path="*" element={<Navigate to={PATIENT_BASE} replace />} />
      </Routes>
    </Layout>
  );
}
