...
const PrefCareType = ({ setView }) => {
  ...
  useEffect(() => {
    ...
    const saved = recipientProfile.careConditions;
    if (Array.isArray(saved)) {
      setSelectedCareTypes(saved.map((item) => String(item)));
    }
  }, [rawUser]);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await updateUserProfileAPI(rawUser._id, {
        recipientProfile: { careConditions: selectedCareTypes },
      });
      if (response?.data) setUser(response.data);
      setView('preferences');
    } catch (error) {
      console.error(error);
      alert('Could not save care type preferences.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto pb-20">
      ...
      <button
        onClick={handleSave}
        disabled={isSaving}
        className="w-full mt-12 py-5 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white rounded-[2rem] font-bold text-base transition-colors disabled:opacity-60"
      >
