...
import { updateUserProfileAPI, uploadImageAPI, getUserProfileAPI } from '../../../utils/api';
...
const ProfileView = ({ setView }) => {
  ...
  const [form, setForm] = useState({
    ...
    pswCertificateUrl: rawUser?.providerProfile?.pswCertificateUrl || '',
    backgroundCheckUrl: rawUser?.providerProfile?.backgroundCheckUrl || '',
    certificateId: rawUser?.providerProfile?.certificateId || '',
    yearsOfExperience: rawUser?.providerProfile?.yearsOfExperience || '',
  });

  const showClinicalCredentials = Boolean(
    form.certificateId ||
    form.yearsOfExperience ||
    form.pswCertificateUrl ||
    form.backgroundCheckUrl,
  );
  ...
  const handleImageFile = async (file) => {
    ...
    if (!validTypes.includes(file.type)) return alert('Please upload PNG, JPG, or WEBP image.');
    if (file.size > 5 * 1024 * 1024) return alert('Image size must be 5MB or less.');
    ...
    try {
      ...
      if (!secureUrl) throw new Error('Image URL missing after upload.');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await updateUserProfileAPI(rawUser._id, {
        ...
        providerProfile: {
          pswCertificateUrl: form.pswCertificateUrl || undefined,
          backgroundCheckUrl: form.backgroundCheckUrl || undefined,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-50 shadow-sm p-10 flex flex-col items-center text-center">
            ...
            <div className="w-full space-y-6 text-left">
              ...
              <div className="flex items-start gap-4">
                ...
                <div><p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Address</p><p className="text-sm font-bold text-gray-800 max-w-[150px]">{`${form.street}, ${form.city}, ${form.province}`.replace(/^,\s*|,\s*$/g, '') || 'Not provided'}</p></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 bg-white rounded-[2.5rem] border border-gray-50 shadow-sm p-12">
          ...
          <div className="space-y-10">
            ...
            {showClinicalCredentials && (
              <div className="space-y-4 pt-4 border-t border-gray-25">
                <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Clinical Credentials</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {form.certificateId ? (
                    <div className="space-y-2">
                      <label className="text-[8px] font-bold text-gray-500 uppercase tracking-widest ml-1">Certification ID</label>
                      <input type="text" value={form.certificateId} onChange={(e) => setForm((prev) => ({ ...prev, certificateId: e.target.value }))} className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-medium focus:bg-white focus:border-purple-100 transition-all outline-none" />
                    </div>
                  ) : null}
                  {form.yearsOfExperience ? (
                    <div className="space-y-2">
                      <label className="text-[8px] font-bold text-gray-500 uppercase tracking-widest ml-1">Years of Experience</label>
                      <input type="number" min="0" value={form.yearsOfExperience} onChange={(e) => setForm((prev) => ({ ...prev, yearsOfExperience: e.target.value }))} className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-medium focus:bg-white focus:border-purple-100 transition-all outline-none" />
                    </div>
                  ) : null}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                  {form.pswCertificateUrl ? (
                    <div className="space-y-2">
                      <label className="text-[8px] font-bold text-gray-500 uppercase tracking-widest ml-1">PSW Certificate</label>
                      <label className="border border-dashed rounded-2xl p-4 flex items-center justify-between">
                        ...
                        <input type="file" accept="image/*,application/pdf" className="hidden" onChange={async (e) => {
                          ...
                          try {
                            ...
                            const res = await uploadImageAPI(b64, 'mypsw/provider-certificates');
                            const url = res?.data?.secureUrl;
                            if (url) setForm(prev => ({ ...prev, pswCertificateUrl: url }));
                          } catch (err) {
                            console.error(err);
                            alert('Upload failed');
                          } finally { setUploadingImage(false); }
                        }} />
                        <button onClick={(ev) => { ev.preventDefault(); ev.currentTarget.previousElementSibling?.click(); }} className="px-4 py-2 bg-purple-600 text-white rounded-xl">Change</button>
                      </label>
                    </div>
                  ) : null}
                  ...
                  {form.backgroundCheckUrl ? (
                    <div className="space-y-2">
                      <label className="text-[8px] font-bold text-gray-500 uppercase tracking-widest ml-1">Background Check</label>
                      <label className="border border-dashed rounded-2xl p-4 flex items-center justify-between">
                        <span className="text-sm">Uploaded</span>
                        <input type="file" accept="image/*,application/pdf" className="hidden" onChange={async (e) => {
                          const file = e.target.files?.[0];
                          ...
                          try {
                            setUploadingImage(true);
                            const b64 = await fileToBase64(file);
                            const res = await uploadImageAPI(b64, 'mypsw/provider-background-checks');
                            const url = res?.data?.secureUrl;
                            if (url) setForm(prev => ({ ...prev, backgroundCheckUrl: url }));
                          } catch (err) {
                            console.error(err);
                            alert('Upload failed');
                          } finally { setUploadingImage(false); }
