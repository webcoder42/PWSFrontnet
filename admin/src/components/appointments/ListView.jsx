...
const ListView = ({ appointments, onBookNew, onReschedule, onViewDetails }) => {
            ...
            {activeAppointments.map((app) => (
              <div key={app.id} className="bg-white p-10 rounded-[2.5rem] border border-gray-50 shadow-[0_20px_50px_rgba(0,0,0,0.02)] relative overflow-hidden group hover:border-purple-200 transition-all cursor-pointer" onClick={() => onViewDetails(app)}>
               ...
               <div className="flex items-center mb-8 relative">
                  <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${app.psw.split(' ')[0]}`} className="w-16 h-16 rounded-2xl object-cover ring-4 ring-white shadow-md shrink-0 group-hover:scale-105 transition-transform" alt={app.psw} />
                  <div className="ml-5">
                     <h4 className="font-bold text-gray-900 text-lg leading-none mb-1.5">{app.psw}</h4>
                     <div className="flex items-center">
                        <span className={`w-2 h-2 rounded-full bg-${app.color === 'emerald' ? 'emerald' : 'purple'}-500 mr-2 shadow-[0_0_10px_rgba(0,0,0,0.1)]`}></span>
                        <p className={`text-[9px] text-${app.color === 'emerald' ? 'emerald' : 'purple'}-600 font-bold uppercase tracking-widest`}>PSW — {app.status}</p>
                     </div>
                  </div>
               </div>
