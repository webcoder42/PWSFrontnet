...
const PrefCareServices = ({ setView }) => {
  ...
  const [selectedCareServices, setSelectedCareServices] = useState(user?.recipientProfile?.servicesNeeded || []);

  const handleSave = () => {
    updateUser({
      recipientProfile: { ...user.recipientProfile, servicesNeeded: selectedCareServices }
    });
    setView('preferences');
  };
