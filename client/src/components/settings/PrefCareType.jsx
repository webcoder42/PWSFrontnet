...
const PrefCareType = ({ setView }) => {
  ...
  const [selectedCareTypes, setSelectedCareTypes] = useState(user?.recipientProfile?.careConditions || []);

  const handleSave = () => {
    updateUser({
      recipientProfile: { ...user.recipientProfile, careConditions: selectedCareTypes }
    });
    setView('preferences');
  };
  ...
  <div className="animate-fade-in max-w-4xl mx-auto pb-20">
    ...
    <button onClick={handleSave} className="w-full mt-12 py-5 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white rounded-[2rem] font-bold text-base transition-colors">Save Changes</button>
  </div>
)};
